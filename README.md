...
- **数据热更新** — 启动时版本检测，强制更新机制从服务端拉取英雄克制数据包
...
1. **服务端地址** — `app/build.gradle` 中 `VERSION_CHECK_URL` / `DATA_DOWNLOAD_BASE_URL` 目前为占位域名，发布前替换为真实服务地址
2. **英雄哈希库** — `app/src/main/assets/kof_data/hero_hashes.json` 为 mock 数据，正式版需在真机截取所有英雄头像并提取 pHash 生成
